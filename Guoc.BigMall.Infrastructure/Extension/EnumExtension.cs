...
namespace Guoc.BigMall.Infrastructure.Extension
{
    public static class EnumExtension
    {
        ...
        public static Dictionary<string, string> GetValueStringToDescription(this Type enumType)
        {
            return enumType.GetEnumInfos().ToDictionary(t => (string)t.Name, t => (string)t.Description);
        }
        public static string GetDescriptionByValue(this Type enuType,int value)
        {
            IList<dynamic> infoList = enuType.GetEnumInfos().ToList();
            dynamic info = infoList.Where(i => i.Value == value).FirstOrDefault();            
            if (info != null)
