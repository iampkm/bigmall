Vue.component("category-selector", {
    template:
        "<span>"
            ...
            + "</el-input>"
            + "<el-dialog title='选择品类' v-bind:width='dialogWidth' custom-class='dialog_size' v-bind:visible.sync='dialogVisible'>"
                + "<el-input v-model='filterText' placeholder='输入品类名称或编码进行过滤' clearable></el-input>"
                + "<el-tree ref='categoryTree' node-key='Key' v-bind:data='treeData' v-bind:props='defaultProps' v-bind:default-checked-keys='value.toString().split(\",\")' v-bind:default-expanded-keys='value.toString().split(\",\")' v-bind:show-checkbox='multiSelect' highlight-current empty-text='没有品类数据。' "
    ...
    props: {
        placeholder: { type: String, default: "选择品类" },
        ...
        url: { type: String, default: "/Category/LoadCategoryTree" },
    ...
    methods: {
        ...
        validate: function (checkedKeys) {
            if (!this.multiSelect && checkedKeys.length > 1) {  //单选
                this.checkedNodes = [];
                this.$message.error("category-selector仅允许单选！");
                return false;
            } else {  //是否可选
                ...
                for (var i = 0; i < matchedNodes.length; i++) {
                    ...
                    if (!this.canSelect(node)) {
                        this.$message.error("category-selector绑定的值包含不可选的节点！");
                        this.checkedNodes = [];
                        return false;
                    }
                }
        ...
        setCheckedNodes: function (keys) {
            ...
            if (this.validate(keys)) {
                //this._setCheckedNodes(this.treeData, keys);
    ...
    created: function () {
        ...
        //    if (nodes) {
        //        nodes.forEach(function (node) {
        //            if (checkedKeys.contains(node.Key))
        //                $this.checkedNodes.push(node);
        //            if (node.Children) {
        //                getDefaultCheckedNodes(node.Children);
        //            }
        //        });
        //    }
        //}
